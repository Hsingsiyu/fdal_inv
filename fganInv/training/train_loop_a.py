    ...
    for epoch in range(epoch_s,max_epoch):
        ...
        while i<len(train_dataloader):
            ...
            while j < D_iters and i < len(train_dataloader):
                ...
                grad_dst=torch.autograd.grad(outputs=dst,inputs=H_hat.parameters(),create_graph=False,retain_graph=True,allow_unused=True)[0]
            ...
            if loss_w_weight>0:
                ...
                grad_w = torch.autograd.grad(outputs=task_loss_w, inputs=itertools.chain(E.parameters(),H.parameters()),
                                             create_graph=False, retain_graph=True, allow_unused=True)[0]
                gradnorm_w= torch.norm(grad_w, dim=None)
                if gradnorm_w > 0:
                    loss_all += torch.div(input=task_loss_w, other=gradnorm_w.detach())
            ...
            dst =  torch.mean(l_s) - torch.mean(phistar_gf(l_t))
            grad_dst = \
            torch.autograd.grad(outputs=dst, inputs=itertools.chain(H.parameters(),E.parameters()), create_graph=False, retain_graph=True,
                                allow_unused=True)[0]
            gradnorm_dst = torch.norm(grad_dst, dim=None)
            if gradnorm_dst > 0:
                loss_all += torch.div(input=dst, other=gradnorm_dst.detach())
            ...
            if writer and config.local_rank==0:
                ...
                writer.add_scalar('gradnorm/w',gradnorm_w.item(), global_step=E_iterations)
