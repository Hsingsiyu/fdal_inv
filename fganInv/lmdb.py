...
if __name__=='__main__':
    import  lmdb
    root='/home/xsy/datasets/cat'
    env=lmdb.open(root, map_size=1099511627776,max_readers=100, readonly=True)
    with env.begin(write=False) as txn:
        cursor = txn.cursor()
        for key, val in cursor:
            print(key)
    # paths_gt, sizes_gt = _get_paths_from_lmdb(root)
    # len(paths_gt)
    # print(paths_gt[0])
    # gt_env = lmdb.open(root, readonly=True, lock=False, readahead=False,meminit=False)
    # img_gt = _read_img_lmdb(gt_env, paths_gt[0], 256)