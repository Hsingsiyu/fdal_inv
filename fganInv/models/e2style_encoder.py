...
class BackboneEncoderFirstStage(Module):
    def __init__(self, num_layers,image_size, mode='ir', input_nc=3,):
        ...
        self.output_layer_3 = Sequential(BatchNorm2d(256),
                                         ...
                                         Flatten(),
                                         Linear(256 * 7 * 7, 512 * self.codelist[2]))
        self.output_layer_4 = Sequential(BatchNorm2d(128),
                                         torch.nn.AdaptiveAvgPool2d((7, 7)),
                                         Flatten(),
                                         Linear(128 * 7 * 7, 512 * self.codelist[1]))
        self.output_layer_5 = Sequential(BatchNorm2d(64),
                                         torch.nn.AdaptiveAvgPool2d((7, 7)),
                                         Flatten(),
                                         Linear(64 * 7 * 7, 512 * self.codelist[0]))
    ...
    def forward(self, x):
        ...
        for l in self.modulelist[:3]:
          x = l(x)
        lc_part_4 = self.output_layer_5(x).view(-1,self.codelist[0], 512)
        for l in self.modulelist[3:7]:
          x = l(x)
        lc_part_3 = self.output_layer_4(x).view(-1, self.codelist[1], 512)
        for l in self.modulelist[7:21]:
          x = l(x)
        lc_part_2 = self.output_layer_3(x).view(-1, self.codelist[2], 512)
